...
export class Snap extends Module {
    ...
    getNearestMagnet(coord) {
        ...
        const magnet = magnets.reduce((p, c) => Math.abs(c.magnet - coord) < Math.abs(p.magnet - coord) ? c : p);
        return Object.assign(Object.assign({}, magnet), { diff: magnet.magnet - coord });
    }
    /** Cancel sticky behavior */
    cancelTransition() {
        var _a;
