...
description: SplitText — split text into lines, words and letters. Resize support, HTML and emoji handling. Vevet.js.
keywords:
  - vevet.splittext
  - vevetjs split text
  - vevet split text
  - text animation
  - split text lines words letters
  - typography
---
...
When `lines` is enabled, SplitText reflows line wrappers on resize.
:::
...
Manual line reflow (only needed when `lines` is `true`):
