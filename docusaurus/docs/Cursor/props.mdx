---
sidebar_position: 0
description: Cursor props — configuration. Vevet.js Cursor API reference.
keywords:
  - cursor props
  - vevet cursor
  - custom cursor
---

import Head from '@docusaurus/Head';

# Props

<Head>
  <title>Cursor Props | vevet.js</title>
</Head>

## Static Props

Static properties are set during initialization and cannot be modified later.

### `container` {#props.container}

- **Type:** `Element | window`
- **Default:** `window`
- The container where the custom cursor is active.  
  Use `window` for full-window activation.

```js
const cursor = new Cursor({
  container: document.getElementById('cursor-container'),
});
```

### `hideNative` {#props.hideNative}

- **Type:** `boolean`
- **Default:** `false`
- Hides the native cursor if set to `true`.

```js
const cursor = new Cursor({
  hideNative: true,
});
```

### `append` {#props.append}

- **Type:** `boolean`
- **Default:** `true`
- Appends the custom cursor element to the container.  
  Use `false` if you need only cursor logic and interactions without the visual DOM element.

```js
const cursor = new Cursor({
  append: false,
});
```

### `behavior` {#props.behavior}

- **Type:** `"default"` | `"path"`
- **Default:** `"default"`
- Defines how the cursor follows the native pointer  
  `default` — Cursor interpolates toward the current pointer position.  
  `path` — Cursor follows a smooth SVG path repeating the pointer trajectory.  
  Use `path` for decorative or expressive cursor effects.

```js
const cursor = new Cursor({
  behavior: 'path',
});
```

### `transformModifier` {#props.transformmodifier}

- **Type:** `(coords: ICursorFullCoords) => string`
- **Default:**
  ```ts
  (coords) => `translate(${coords.x}px, ${coords.y}px)`;
  ```
- Modifier for the cursor transform.

```js
const cursor = new Cursor({
  transformModifier: (coords) =>
    `translate(${coords.x}px, ${coords.y}px) rotate(${coords.angle}deg)`,
});
```

## Mutable Props

Mutable properties can be updated at runtime using `.updateProps()`.

:::tip Adaptive props
To apply different values by viewport or device, use **[Responsive](/docs/base/Responsive/#example-with-module)** with this component as the source. Props will update automatically when breakpoints change.
:::

### `enabled` {#props.enabled}

- **Type:** `boolean`
- **Default:** `true`
- Enables or disables the custom cursor interactions.

```js
const cursor = new Cursor({
  enabled: false,
});

// change value
cursor.updateProps({
  enabled: true,
});
```

### `width` {#props.width}

- **Type:** `number`
- **Default:** `50`
- The initial width of the custom cursor. Supports css units like `px`, `rem`, `vw`, `vh`, `svh`.

```js
const cursor = new Cursor({
  width: 50,
});

// change value
cursor.updateProps({
  width: '3rem',
});
```

### `height` {#props.height}

- **Type:** `number`
- **Default:** `50`
- The initial height of the custom cursor. Supports css units like `px`, `rem`, `vw`, `vh`, `svh`.

```js
const cursor = new Cursor({
  height: 50,
});

// change value
cursor.updateProps({
  height: '3rem',
});
```

### `lerp` {#props.lerp}

- **Type:** `number`
- **Default:** `0.2`
- Linear interpolation factor for smooth cursor movement.  
  The value must be between `0` and `1`, with higher values indicating faster movement.

```js
const cursor = new Cursor({
  lerp: 0.2,
});

// change value
cursor.updateProps({
  lerp: 0.05,
});
```

### `angleLerpMultiplier` {#props.angleLerpMultiplier}

- **Type:** `number`
- **Default:** `2`
- Linear interpolation factor for smooth cursor rotation.  
  This value multiplies the `lerp` value.

```js
const cursor = new Cursor({
  angleLerpMultiplier: 2,
});

// change value
cursor.updateProps({
  angleLerpMultiplier: 3,
});
```

### `velocityThreshold` {#props.velocityThreshold}

- **Type:** `number`
- **Default:** `150`
- Velocity normalization threshold in pixels.  
  Used to convert pointer movement deltas into normalized `velocity`, `impulseX`, and `impulseY`.
- Higher values reduce sensitivity (slower velocity growth); lower values increase it.

```js
const cursor = new Cursor({
  velocityThreshold: 150,
});

// change value
cursor.updateProps({
  velocityThreshold: 250,
});
```

### `autoStop` {#props.autoStop}

- **Type:** `boolean`
- **Default:** `true`
- Automatically stops the internal `requestAnimationFrame` loop when cursor position and size reach target values.  
  Set to `false` if you need continuous rendering (e.g. for shaders or custom effects).

```js
const cursor = new Cursor({
  autoStop: true,
});

// change value
cursor.updateProps({
  autoStop: false,
});
```
