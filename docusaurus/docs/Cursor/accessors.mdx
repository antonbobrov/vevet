...
Type: `{ height: number; width: number; x: number; y: number; angle: number; velocity: number; impulseX: number; impulseY: number }`
...
cursor.coords.impulseX;
cursor.coords.impulseY;
```
...
cursor.targetCoords.impulseX;
cursor.targetCoords.impulseY;
```
