...
description: CanvasMedia — Vevet.js pre-rendering of media into canvas layer for animations, effects and transitions.
keywords:
  - vevet.canvasmedia
  - vevetjs canvas media
  - vevet canvas media
  - canvas video image
  - pre-render
  - rasterize
...
<VevetDemo id="vEEBroM" height={500} />
...
`render` also fires after `resize()` — for example when `rule` or canvas size changes.
