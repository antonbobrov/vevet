---
sidebar_position: 3
description: Snap callbacks — onActiveSlide, onSwipe and more. Vevet.js Snap API reference.
keywords:
  - vevet.snap callbacks
  - vevetjs snap callbacks
  - vevet snap callbacks
  - snap callbacks
  - carousel events
---

import Head from '@docusaurus/Head';

# Callbacks

<Head>
  <title>Snap Callbacks | vevet.js</title>
</Head>

:::note
All **[Module callbacks](/docs/base/Module/#callbacks)** are available in this class.
:::

### `activeSlide`

Triggered when the active slide changes.  
The callback receives the active slide instance. See **[SnapSlide](./slide)**.

```ts
const snap = new Snap({
  container,
  onActiveSlide: (slide) => console.log(slide.index),
});
```

or:

```ts
const destruct = snap.on('activeSlide', (slide) => console.log(slide.index));

// Cancel the callback
destruct();
```

### `rafPlay`

Triggered when the requestAnimationFrame loop starts — when the track begins interpolating toward the target.

```ts
const snap = new Snap({
  container,
  onRafPlay: () => console.log('play'),
});
```

or:

```ts
const destruct = snap.on('rafPlay', () => console.log('play'));

// Cancel the callback
destruct();
```

### `rafFrame`

Triggered on each requestAnimationFrame tick while the track interpolates toward the target.

- Skipped during Timeline transitions — use [`timelineUpdate`](#timelineupdate) instead.
- Fires before [`update`](#update) on the same tick.
- Does not fire when `render()` is called manually (e.g. on resize).

```ts
const snap = new Snap({
  container,
  onRafFrame: () => console.log('rafFrame'),
});
```

or:

```ts
const destruct = snap.on('rafFrame', () => console.log('rafFrame'));

// Cancel the callback
destruct();
```

### `rafPause`

Triggered when the requestAnimationFrame loop pauses — when the track reaches the target and interpolation stops.

```ts
const snap = new Snap({
  container,
  onRafPause: () => console.log('pause'),
});
```

or:

```ts
const destruct = snap.on('rafPause', () => console.log('pause'));

// Cancel the callback
destruct();
```

### `reflow`

Triggered when the carousel recalculates slides, sizes, or magnets.

```ts
const snap = new Snap({
  container,
  onReflow: () => console.log('reflow'),
});
```

or:

```ts
const destruct = snap.on('reflow', () => console.log('reflow'));

// Cancel the callback
destruct();
```

### `resize`

Triggered when the carousel is resized.

```ts
const snap = new Snap({
  container,
  onResize: () => console.log('resize'),
});
```

or:

```ts
const destruct = snap.on('resize', () => console.log('resize'));

// Cancel the callback
destruct();
```

### `swipe`

Triggered during swipe movement.

```ts
const snap = new Snap({
  container,
  onSwipe: (coord) => console.log(coord),
});
```

or:

```ts
const destruct = snap.on('swipe', (coord) => console.log(coord));

// Cancel the callback
destruct();
```

### `swipeStart`

Triggered when a swipe starts.

```ts
const snap = new Snap({
  container,
  onSwipeStart: (coord) => console.log(coord),
});
```

or:

```ts
const destruct = snap.on('swipeStart', (coord) => console.log(coord));

// Cancel the callback
destruct();
```

### `swipeEnd`

Triggered when a swipe ends.

```ts
const snap = new Snap({
  container,
  onSwipeEnd: (coord) => console.log(coord),
});
```

or:

```ts
const destruct = snap.on('swipeEnd', (coord) => console.log(coord));

// Cancel the callback
destruct();
```

### `timelineStart`

Triggered on timeline animation start.

```ts
const snap = new Snap({
  container,
  onTimelineStart: () => console.log('start'),
});
```

or:

```ts
const destruct = snap.on('timelineStart', () => console.log('start'));

// Cancel the callback
destruct();
```

### `timelineUpdate`

Triggered on timeline animation progress update.

```ts
const snap = new Snap({
  container,
  onTimelineUpdate: ({ progress, eased }) => console.log(progress, eased),
});
```

or:

```ts
const destruct = snap.on('timelineUpdate', ({ progress, eased }) => {
  console.log(progress, eased);
});

// Cancel the callback
destruct();
```

### `timelineEnd`

Triggered when a timeline animation ends.

```ts
const snap = new Snap({
  container,
  onTimelineEnd: () => console.log('end'),
});
```

or:

```ts
const destruct = snap.on('timelineEnd', () => console.log('end'));

// Cancel the callback
destruct();
```

### `update`

Triggered on every carousel coordinate update — after slide coords, magnets, and friction are applied.

During RAF interpolation, fires on the same tick as [`rafFrame`](#rafframe), immediately after it.  
During Timeline transitions, fires from [`timelineUpdate`](#timelineupdate) instead.

```ts
const snap = new Snap({
  container,
  onUpdate: () => console.log('update'),
});
```

or:

```ts
const destruct = snap.on('update', () => console.log('update'));

// Cancel the callback
destruct();
```

### `idle`

Triggered when the carousel becomes idle — not swiping, interpolating, or transitioning.

```ts
const snap = new Snap({
  container,
  onIdle: () => console.log('idle'),
});
```

or:

```ts
const destruct = snap.on('idle', () => console.log('idle'));

// Cancel the callback
destruct();
```

### `wheel`

Triggered on mouse wheel input.

```ts
const snap = new Snap({
  container,
  onWheel: (evt) => console.log(evt),
});
```

or:

```ts
const destruct = snap.on('wheel', (evt) => console.log(evt));

// Cancel the callback
destruct();
```

### `wheelStart`

Triggered when wheel input starts.

```ts
const snap = new Snap({
  container,
  onWheelStart: () => console.log('start'),
});
```

or:

```ts
const destruct = snap.on('wheelStart', () => console.log('start'));

// Cancel the callback
destruct();
```

### `wheelEnd`

Triggered when wheel input ends.

```ts
const snap = new Snap({
  container,
  onWheelEnd: () => console.log('end'),
});
```

or:

```ts
const destruct = snap.on('wheelEnd', () => console.log('end'));

// Cancel the callback
destruct();
```

### `swipeInertiaStart`

Triggered when swipe inertia starts.

```ts
const snap = new Snap({
  container,
  onSwipeInertiaStart: () => console.log('start'),
});
```

or:

```ts
const destruct = snap.on('swipeInertiaStart', () => console.log('start'));

// Cancel the callback
destruct();
```

### `swipeInertiaEnd`

Triggered when swipe inertia ends.

```ts
const snap = new Snap({
  container,
  onSwipeInertiaEnd: () => console.log('end'),
});
```

or:

```ts
const destruct = snap.on('swipeInertiaEnd', () => console.log('end'));

// Cancel the callback
destruct();
```

### `swipeInertiaCancel`

Triggered when swipe inertia is canceled.

```ts
const snap = new Snap({
  container,
  onSwipeInertiaCancel: () => console.log('cancel'),
});
```

or:

```ts
const destruct = snap.on('swipeInertiaCancel', () => console.log('cancel'));

// Cancel the callback
destruct();
```

### `swipeInertiaFail`

Triggered when swipe inertia fails to start.

```ts
const snap = new Snap({
  container,
  onSwipeInertiaFail: () => console.log('fail'),
});
```

or:

```ts
const destruct = snap.on('swipeInertiaFail', () => console.log('fail'));

// Cancel the callback
destruct();
```
