---
sidebar_position: 0
description: Snap props — carousel configuration. Vevet.js Snap API reference.
keywords:
  - snap props
  - vevet snap
  - carousel props
...
- **Default:** `'start'`
- Aligns slides and snap magnets along the scroll axis:
...
- **Type:** `boolean`
- **Default:** `false`
- **Status:** deprecated — use [`align`](#props.align) instead (`centered: true` is equivalent to `align: 'center'`). If both are set, `centered` takes precedence.
...
### `intervalDirection` {#props.intervalDirection}
