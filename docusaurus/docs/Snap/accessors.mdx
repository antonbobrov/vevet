---
sidebar_position: 1
description: Snap accessors — index, progress, state getters. Vevet.js Snap API reference.
keywords:
  - snap accessors
  - vevet snap
---
...
Returns the resolved slide magnets origin. Reflects [`origin`](./props#props.origin) and the deprecated [`centered`](./props#props.centered) prop (`centered: true` → `'center'`).
...
Snap requires at least one slide during initialization and throws when no slides are found.
...
Returns the impulse applied from the current value to the target, caused by swipe or wheel input.  
