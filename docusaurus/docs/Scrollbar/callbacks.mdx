---
sidebar_position: 3
description: Scrollbar callbacks — scroll events. Vevet.js Scrollbar API reference.
keywords:
  - vevet.scrollbar callbacks
  - vevetjs scrollbar callbacks
  - vevet scrollbar callbacks
  - scrollbar callbacks
---

import Head from '@docusaurus/Head';

# Callbacks

<Head>
  <title>Scrollbar Callbacks | vevet.js</title>
</Head>

:::note
All **[Module callbacks](/docs/base/Module/#callbacks)** are available in this class.
:::

### `update`

Triggered when the scrollbar updates its position.

```ts
const instance = new Scrollbar({
  onUpdate: () => console.log('update'),
});
```

or:

```ts
const destruct = instance.on('update', () => console.log('update'));

// Cancel the callback
destruct();
```

### `resize`

Triggered when the scrollbar resizes.

```ts
const instance = new Scrollbar({
  onResize: () => console.log('resize'),
});
```

or:

```ts
const destruct = instance.on('resize', () => console.log('resize'));

// Cancel the callback
destruct();
```

### `show`

Triggered when the scrollbar becomes visible during scroll activity (`autoHide`).

Requires `autoHide: true`. Fired after a short delay once scroll position changes.

```ts
const instance = new Scrollbar({
  onShow: () => console.log('show'),
});
```

or:

```ts
const destruct = instance.on('show', () => console.log('show'));

// Cancel the callback
destruct();
```

### `hide`

Triggered when the scrollbar hides after scroll inactivity (`autoHide`).

```ts
const instance = new Scrollbar({
  onHide: () => console.log('hide'),
});
```

or:

```ts
const destruct = instance.on('hide', () => console.log('hide'));

// Cancel the callback
destruct();
```

### `swipeStart`

Triggered when swipe starts.

```ts
const instance = new Scrollbar({
  onSwipeStart: (coords) => console.log(coords),
});
```

or:

```ts
const destruct = instance.on('swipeStart', (coords) => console.log(coords));

// Cancel the callback
destruct();
```

### `swipe`

Triggered during swipe movement.

```ts
const instance = new Scrollbar({
  onSwipe: (coords) => console.log(coords),
});
```

or:

```ts
const destruct = instance.on('swipe', (coords) => console.log(coords));

// Cancel the callback
destruct();
```

### `swipeEnd`

Triggered when swipe ends.

```ts
const instance = new Scrollbar({
  onSwipeEnd: (coords) => console.log(coords),
});
```

or:

```ts
const destruct = instance.on('swipeEnd', (coords) => console.log(coords));

// Cancel the callback
destruct();
```
