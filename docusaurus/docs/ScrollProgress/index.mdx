...
keywords:
  - vevet.scrollprogress
  - vevetjs scroll progress
...
Tracks scroll progress of a section relative to the viewport or a custom scroll root.  
Useful for parallax, scrubbed animations, and enter/leave effects.

## Initialization

```js
import { ScrollProgress, vevet } from 'vevet';
...
const progress = new ScrollProgress({
  section,
  onUpdate: () => {
    const { y } = progress.moveProgress;
    section.style.setProperty('--p', String(y));
  },
});
```

Destroying the instance removes the scroll listener and disconnects the `IntersectionObserver`:
...
// Force update even when the section is not visible (optimized mode)
instance.update(true);
```

### `destroy`

Releases the scroll listener and disconnects the `IntersectionObserver`.
