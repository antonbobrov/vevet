---
sidebar_position: 3
description: Raf callbacks — frame events. Vevet.js Raf API reference.
keywords:
  - vevet.raf callbacks
  - vevetjs raf callbacks
  - vevet raf callbacks
  - raf callbacks
---

import Head from '@docusaurus/Head';

# Callbacks

<Head>
  <title>Raf Callbacks | vevet.js</title>
</Head>

:::note
All **[Module callbacks](/docs/base/Module/#callbacks)** are available in this class.
:::

### `play`

Triggered when the animation starts.

```ts
const raf = new Raf({
  onPlay: () => console.log('play'),
});
```

or:

```ts
const destruct = raf.on('play', () => console.log('play'));

// Cancel the callback
destruct();
```

### `pause`

Triggered when the animation is paused.

```ts
const raf = new Raf({
  onPause: () => console.log('pause'),
});
```

or:

```ts
const destruct = raf.on('pause', () => console.log('pause'));

// Cancel the callback
destruct();
```

### `toggle`

Triggered when the play/pause state toggles.

```ts
const raf = new Raf({
  onToggle: () => console.log('toggle'),
});
```

or:

```ts
const destruct = raf.on('toggle', () => console.log('toggle'));

// Cancel the callback
destruct();
```

### `frame`

Triggered on every animation frame.

```ts
const raf = new Raf({
  onFrame: ({ fps, fpsFactor, duration, lerpFactor }) => {
    console.log('current FPS', fps);
    console.log('scaling coefficient based on a 60 FPS target', fpsFactor);
    console.log('duration of the last frame in ms', duration);
    console.log('FPS-independent ease', lerpFactor(0.1));
  },
});
```

or:

```ts
const destruct = raf.on('frame', ({ fps, fpsFactor, duration, lerpFactor }) => {
  console.log('current FPS', fps);
  console.log('scaling coefficient based on a 60 FPS target', fpsFactor);
  console.log('duration of the last frame in ms', duration);
  console.log('FPS-independent ease', lerpFactor(0.1));
});

// Cancel the callback
destruct();
```
